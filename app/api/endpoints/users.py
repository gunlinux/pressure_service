...
def create_user(user: UserCreate, db: Annotated[Session, Depends(get_db)]):
    db_user = user_service.get_user_by_telegram_nickname(
        db, telegram_nickname=user.telegram_nickname
    )
    if db_user:
        raise HTTPException(
            status_code=400, detail='Telegram nickname already registered'
        )
    try:
...
def create_pressure_measurements_for_user(
    ...
    return pressure_service.create_measurements(
        db=db, user_id=user_id, measurements_data=measurements
    )
